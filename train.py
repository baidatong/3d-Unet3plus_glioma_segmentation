...
from unet3d_triplePlus import (unet3d,Loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu', default='0',help='comma separated list of GPU(s) to use. Default to all availalbe ones')
    parser.add_argument('--load', help='load model for evaluation or training') #default='train_log/unet3d/model-120000',
    ...
    if args.visualize or args.evaluate:
        if config.DYNAMIC_SHAPE_PRED:
            ...
            pred = OfflinePredictor(PredictConfig(
                    ...
                    input_names=['image'],
                    output_names=get_model_output_names()))
            # autotune is too slow for infsegment_one_imageerence
            os.environ['TF_CUDNN_USE_AUTOTUNE'] = '0'
            assert args.load
        ...
        cfg = TrainConfig(
            ...
            callbacks=[
                ...
                ScheduledHyperParamSetter('learning_rate', 
                    [(200, config.BASE_LR*0.1),
                    (300, config.BASE_LR*0.01),(400, config.BASE_LR*0.001)],
                ),
                #EvalCallback(),
