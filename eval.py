...
def batch_segmentation(temp_imgs, model_func, data_shape=[19, 180, 160]):
    ...
    for center_slice in range(int(label_shape[0] / 2), D + int(label_shape[0] / 2), label_shape[0]):
        for center_slice2 in range(int(label_shape[1] / 2), H + int(label_shape[1] / 2), label_shape[1]):
            for center_slice3 in range(int(label_shape[2] / 2), W + int(label_shape[2] / 2), label_shape[2]):
                center_slice = min(center_slice, D - int(label_shape[0] / 2))
                center_slice2 = min(center_slice2, H - int(label_shape[1] / 2))
                center_slice3 = min(center_slice3, W - int(label_shape[2] / 2))


                sub_image_batch = []
                for chn in range(data_channel):
                    temp_input_center = [center_slice, center_slice2, center_slice3]
                    sub_image = extract_roi_from_volume(temp_imgs[chn], temp_input_center, data_shape, fill="zero")
                    sub_image_batch.append(sub_image)
                ind_3d.append([center_slice, center_slice2, center_slice3])
                sub_image_batch = np.asanyarray(sub_image_batch, np.float32)  # [4,180,160]
                sub_image_batches.append(sub_image_batch)  # [14,4,d,h,w]
        #print(len(sub_image_batches))
    ...
    for mini_batch_idx in range(max_mini_batch):
        ...
        for batch_idx in range(prob_mini_batch1.shape[0]):
            ...
            #center_slice = min(center_slice, D - int(label_shape[0] / 2))
            temp_input_center = ind_3d[mini_batch_idx*batch_size+batch_idx]

            #temp_inpubatch_idxt_center = [center_slice, input_center[1], input_center[2], int(class_num / 2)]

            temp_input_center.append(int(class_num/2))
...
def eval_brats(df, detect_func, with_gt=True):
    ...
    with tqdm.tqdm(total=df.size(), **get_tqdm_kwargs()) as pbar:
        for filename, image_id, data in df.get_data():
            ...
            gt = load_nifty_volume_as_array("{}/{}_seg.nii.gz".format(filename, filename.split(sep='/')[-1]))
            gts.append(gt)
